    ...
    return (
      <div className="max-w-md mx-auto text-center py-20 card border-dashed border-2 bg-slate-50/50 dark:bg-slate-800/50">
        ...
        <h2 className="font-display font-bold text-2xl text-slate-800 dark:text-white mb-2">
          {ur('No grocery list yet', 'ابھی تک گروسری فہرست نہیں')}
        </h2>
        <p className="text-slate-500 dark:text-slate-400 mb-6 text-base">
          {ur('Generate a meal plan first to get your grocery list.', 'گروسری فہرست حاصل کرنے کے لیے پہلے کھانے کا منصوبہ بنائیں۔')}
        </p>
        <Link to="/meal-plan" className="btn-primary py-3 px-8 inline-flex">
          {ur('Generate Meal Plan →', 'کھانے کا منصوبہ بنائیں ←')}
        </Link>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-2xl mx-auto animate-fade-in" style={isUrdu ? { fontFamily: "'Noto Nastaliq Urdu', serif" } : {}}>
      <div className="flex items-center justify-between flex-wrap gap-3 mb-6">
        <div>
          <h1 className="page-title">{ur('Grocery List', 'گروسری فہرست')}</h1>
          <p className="page-subtitle">{ur('Based on your latest 7-day meal plan', 'آپ کے تازہ ترین 7 روزہ کھانے کے منصوبے پر مبنی')}</p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => window.print()} className="btn-amber py-2 px-5 text-sm font-bold flex items-center gap-2">
            <Printer className="w-4 h-4" /> {ur('Print / Export PDF', 'پرنٹ / PDF')}
          </button>
          <button onClick={() => dispatch(clearChecked())} className="btn-secondary py-2 px-5 text-sm">
            <RotateCcw className="w-4 h-4" /> {ur('Reset Checks', 'ری سیٹ')}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="card text-center">
          ...
          <p className="text-xs text-slate-400">{ur('Total Items', 'کل اشیاء')}</p>
        </div>
        <div className="card text-center">
          <p className="text-2xl font-bold font-display text-blue-600">{checkedCount}</p>
          <p className="text-xs text-slate-400">{ur('Purchased', 'خریدا گیا')}</p>
        </div>
        <div className="card text-center">
          <p className="text-2xl font-bold font-display text-amber-600">₨{totalCost}</p>
          <p className="text-xs text-slate-400">{ur('Total Cost', 'کل لاگت')}</p>
        </div>
      </div>
      ...
      {totalItems > 0 && (
        <div className="card mb-6">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-slate-700 dark:text-slate-200">{ur('Shopping Progress', 'خریداری کی پیش رفت')}</span>
            <span className="text-sm font-bold text-emerald-600">{checkedCount}/{totalItems}</span>
          </div>
          <div className="h-3 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
            <div className="h-full bg-emerald-500 rounded-full transition-all duration-500" style={{ width: `${totalItems ? (checkedCount / totalItems) * 100 : 0}%` }} />
          </div>
          {checkedCount === totalItems && totalItems > 0 && (
            <p className="text-emerald-600 font-semibold text-sm mt-3 text-center flex items-center justify-center gap-1.5">
              <CheckCircle2 className="w-4 h-4" /> {ur('All items purchased!', 'تمام اشیاء خریدی جا چکی ہیں!')}
            </p>
          )}
        </div>
      )}
      ...
      {loading ? (
        <div className="card text-center py-12">
          ...
          <p className="text-slate-400 mt-3 text-sm">{ur('Loading grocery list...', 'گروسری فہرست لوڈ ہو رہی ہے...')}</p>
        </div>
      ) : items.length === 0 ? (
        <div className="card text-center py-16 border-dashed border-2 bg-slate-50/50 dark:bg-slate-800/50">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-base">{ur('Your grocery list is empty', 'آپ کی گروسری فہرست خالی ہے')}</p>
        </div>
      ) : (
        <div className="space-y-4">
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center justify-between p-3 rounded-xl cursor-pointer transition-all duration-150 ${
                        checked ? 'bg-emerald-50 dark:bg-emerald-900/20 opacity-60' : 'bg-slate-50 dark:bg-slate-700/50 hover:bg-slate-100 dark:hover:bg-slate-700'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${checked ? 'bg-emerald-500 border-emerald-500' : 'border-slate-300 dark:border-slate-500'}`}>
                          {checked && <Check className="w-3 h-3 text-white" />}
                        </div>
                        <span className={`text-sm font-medium ${checked ? 'line-through text-slate-400' : 'text-slate-700 dark:text-slate-200'}`}>{item.name}</span>
                      </div>
          ...
          <div className="card bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 border-amber-100 dark:border-amber-800">
            <div className="flex items-center justify-between">
              <div>
                <p className="font-display font-bold text-slate-800 dark:text-white">{ur('Total Weekly Cost', 'کل ہفتہ وار لاگت')}</p>
                <p className="text-xs text-slate-500 mt-0.5">{ur('For your 7-day meal plan', 'آپ کے 7 روزہ کھانے کے منصوبے کے لیے')}</p>
              </div>
            ...
            <div className="mt-3 pt-3 border-t border-amber-100 dark:border-amber-800 flex justify-between text-sm">
              <span className="text-slate-500">{ur('Daily average', 'یومیہ اوسط')}</span>
