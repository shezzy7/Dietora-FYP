...
export default function FeedbackPage() {
  ...
  if (submitted) {
    return (
      <div className="max-w-md mx-auto text-center py-20 animate-fade-in card border-dashed border-2 bg-slate-50/50 dark:bg-slate-800/50">
        ...
        <p className="text-slate-500 dark:text-slate-400 mb-6 text-base">Your feedback helps us improve DIETORA for everyone.</p>
        <button onClick={() => { setSubmitted(false); setForm({ rating: 5, category: 'General Feedback', message: '' }) }}
          className="btn-primary py-3 px-8 inline-flex text-base">Submit Another</button>
      </div>
    )
  }
    ...
    <div className="max-w-2xl mx-auto animate-fade-in">
              ...
              {RATINGS.map((r) => (
                <button
                  ...
                  className={`w-12 h-12 rounded-2xl border-2 flex items-center justify-center transition-all duration-200 ${
                    r <= form.rating
                      ? 'border-amber-400 bg-amber-50 dark:bg-amber-900/20 text-amber-500 shadow-sm'
                      : 'border-slate-200 dark:border-slate-600 text-slate-300 hover:border-amber-300'
                  }`}
                >
                  <Star className={`w-6 h-6 ${r <= form.rating ? 'fill-current' : ''}`} />
                </button>
              ))}
              <span className="self-center ml-3 text-sm font-bold text-amber-600 uppercase tracking-wide">
      ...
      <div className="card mt-6 bg-emerald-50 dark:bg-emerald-900/20 border-emerald-100 dark:border-emerald-800">
        <p className="text-sm text-emerald-700 dark:text-emerald-400 font-bold mb-1 flex items-center gap-2">
          <Mailbox className="w-4 h-4" /> How we use your feedback
        </p>
