...
export default function Navbar() {
  ...
  const navLinks = [
    { to: '/',       labelEn: 'Home',       labelUr: 'ہوم',            end: true },
    { to: '/about',  labelEn: 'About',      labelUr: 'ہمارے بارے میں' },
    { to: '/faq',    labelEn: 'FAQ',        labelUr: 'سوالات' },
  ]
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-6">
            {navLinks.map((link) => (
              <NavLink key={link.to} to={link.to} end={link.end}
                className={({ isActive }) => isActive ? 'text-emerald-600 font-semibold text-sm' : 'nav-link'}>
            ...
            <a
              href="mailto:dietora.gcuf@gmail.com"
          ...
          <div className="flex items-center gap-3">
            ...
            {user ? (
              ...
              <div className="hidden md:flex items-center gap-2">
                <Link to="/login"    className="btn-secondary py-2 px-4 text-sm">{isUrdu ? 'لاگ ان' : 'Login'}</Link>
                <Link to="/register" className="btn-primary py-2 px-4 text-sm">{isUrdu ? 'شروع کریں' : 'Get Started'}</Link>
              </div>
            )}

            {/* Mobile menu button */}
            <button className="md:hidden p-2 rounded-lg text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700"
        ...
        {mobileOpen && (
          <div className="md:hidden pb-4 space-y-2 animate-slide-up relative z-50">
            <div className="p-2 bg-white/50 dark:bg-slate-900/50 rounded-2xl border border-slate-200/50 dark:border-slate-700/50 backdrop-blur-md">
              {navLinks.map((link) => (
                <NavLink key={link.to} to={link.to} end={link.end}
                  className="block py-3 px-4 rounded-xl nav-link mb-1 hover:bg-slate-100/50 dark:hover:bg-slate-800/50"
              ...
              {/* Contact Us mobile */}
              <a href="mailto:dietora.gcuf@gmail.com"
                className="flex items-center gap-2 py-3 px-4 rounded-xl nav-link mb-2 hover:bg-slate-100/50 dark:hover:bg-slate-800/50"
                onClick={() => setMobileOpen(false)}>
              ...
              {!user && (
                <div className="flex gap-2 pt-2 border-t border-slate-200/50 dark:border-slate-700/50 px-2 mt-2">
                  <Link to="/login"    className="btn-secondary py-2.5 px-4 text-sm flex-1 text-center" onClick={() => setMobileOpen(false)}>{isUrdu ? 'لاگ ان' : 'Login'}</Link>
                  <Link to="/register" className="btn-primary  py-2.5 px-4 text-sm flex-1 text-center" onClick={() => setMobileOpen(false)}>{isUrdu ? 'شروع کریں' : 'Get Started'}</Link>
                </div>
              )}
            </div>
          </div>
